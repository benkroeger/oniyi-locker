...
const {
  STATE_LOCKED, STATE_UNLOCKED, STATE_RELEASED_WITH_MESSAGE, FACTORY_PARAMS_WHITELIST,
} = require('./constants');
const {
  InvalidParamsError,
  LockTimeoutError,
...
function acquireLock(params = {}, done) {
  const {
    client, key, unlockToken, expiresAfter,
  } = params;
  client.set(key, unlockToken, 'PX', expiresAfter, 'NX', (setLockError, result) => {
    if (setLockError) {
      done(setLockError);
      return;
    }
...
function attemptAcquire(params = {}, done) {
  ...
  const acquireParams = _.pick(params, ['client', 'key', 'unlockToken', 'expiresAfter']);
...
function subscribeToReuseData(params = {}, done) {
  ...
  const timeout = setTimeout(() => {
    logger.debug('waited %dms for lock "%s" to be released, aborted', maxWait, key);
    client.unsubscribe();
    client.disconnect();
...
function lockerFactory(factoryParams = {}) {
  ...
  if (!_.isNumber(options.maxAttempts) || options.maxAttempts < 1) {
    throw new InvalidParamsError(
      '.maxAttempts',
      'number > 0',
      `type: ${typeof options.maxAttempts}, val: ${_.toString(options.maxAttempts)}`
    );
  }
