...
function makeRedisClient(params) {
  ...
  if (params.cluster && _.isPlainObject(params.cluster)) {
    ...
    logger.debug(
      'creating redis cluster client with startupNodes "%j", clusterOptions "%j" and redisOptions "%j"',
      startupNodes,
      clusterOptions,
      redisOptions,
    );
    ...
    logger.debug(
      'creating redis standalone client with options "%j"',
      redisOptions,
    );
